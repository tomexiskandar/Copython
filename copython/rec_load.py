...
class RecordLoader:
    def __init__(self,copy_target,target_metadata,mapped_target_columnnames_list,insert_method,max_row_per_batch):
        ...
        self.quoted_mapped_target_columnnames_list = self.quote_columnnames_list(mapped_target_columnnames_list)
        self.insert_method = insert_method
        self.record_list = []
        self.param_marker = ""
        self.param_value_list = []

    def quote_columnnames_list(self,mapped_target_columnnames_list):
        return ['"{}"'.format(x) for x in mapped_target_columnnames_list]
    ...
    def add_record(self,record):
        ...
        if self.insert_method == "prepared":
            batch_count = int(len(self.record_list)/len(self.quoted_mapped_target_columnnames_list))
        else:#reserved for batch
        ...
        if(batch_count % self.max_row_per_batch == 0 or record == False):
            if len(self.record_list) > 0:
                if self.insert_method == "prepared":
                    if record:
                        _param_markers = self.gen_row_param_markers(len(self.quoted_mapped_target_columnnames_list),self.max_row_per_batch)
                    else:
                        _param_markers = self.gen_row_param_markers(len(self.quoted_mapped_target_columnnames_list),batch_count)
                    #print("----")
                    #print(_param_markers)
                    prepared_stmt = """INSERT INTO {}.{} ({}) VALUES {}
                                    """.format(self.target_metadata.schema_name,self.target_metadata.table_name,",".join(self.quoted_mapped_target_columnnames_list),_param_markers)
                    #print(prepared_stmt)
                    ...
                    batch_stmt = """INSERT INTO {}.{} ({}) VALUES {}
                                """.format(self.target_metadata.schema_name,self.target_metadata.table_name,",".join(self.quoted_mapped_target_columnnames_list),",".join(self.record_list))
