...
def copyx(config,insert_method = 'batch',multi_process = False):
    print("hello I'm still in here")
    quit()
...
def execute_copy(copy):
    ...
    if copy.target.__class__.__name__ == "CSVConf":
        ...
        if _is_trg_tbl_existence:
            conn = pyodbc.connect(str(copy.target.conn_str))
            conn.execute("drop table {}.{}".format(copy.target.schema_name,copy.target.table_name))
            conn.commit()
            print("table {}.{} deleted!".format(copy.target.schema_name,copy.target.table_name))
        #!!!! test end
        ...
        if _is_trg_tbl_existence:
            ...
            metadata.create_simple_sql_table(copy.target,type_info_map,src_md)
