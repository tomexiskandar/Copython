...
def get_tables_with_record(data,paths_dict,data_injection_lod = None):
    ...
    for path, val in traverse(data):
        ...
        if len(path) > 1:
            if path[1] - rownum == 1: # if rownum has increase 1
    ...
    for tab,record in record_dict.items():
        ...
        #     print(' ',record)
        pass
    for tab,record in tables_dict.items():
        print(tab,record)
    return tables_dict
...
def process(data,session_dt_string = None):
    ...
    #print('---------------')
    #path_toslice = ['Results','#']
    path_toslice = "Results/#"
    # print(path_toslice2.split("/"))
    # quit(0)
    artg_records = app_test.slice(data,path_toslice)
    # for i,rec in enumerate(artg_records):
    #     print('-----',i)
        #print(rec)

    # artg_record = artg_records[0]
    # print(artg_record)
    # quit(0)


    # tab = app_test.Fly_Table('artg_entry')
    # print(tab.name)
    # tab.parent = 'artg'
    # print(tab.parent)
    path = r"C:\Users\60145210\Documents\Projects\copython\test\artg_project\path_totable.json"

    ptt = app_test.get_json_fromfile(path)
    # for tab in ptt["tables"]:
    #     print(tab["name"],tab["path_toslice"])
        # for row in tab["path"]:
        #     print(" ",row)
        # for k,v in tab["colmap"].items():
        #     print(" ",k,"->",v)
    # create artg_entry table (a list of lod)

    print(ptt["tables"][1]["name"])
